...
{
	unsigned long *pData = 0, *pTmp = 0;
	int i = 0;
	bool bIsLittleEndian = PVRTIsLittleEndian();
	...
	pData = (unsigned long *) (pPVR + (pPVR[0] / sizeof(unsigned long)));

	if(!bIsLittleEndian)
	{
		pTmp = new unsigned long[128 * 64];

		for(i = 0; i < 128 * 64; ++i)
			pTmp[i] = ((pData[i]&0xFF000000)>>16) | ((pData[i]&0x00FF0000)>>15) | 0x00000001 |
				  ((pData[i]&0x0000FF00)<<16) | ((pData[i]&0x000000FF)<<17) | 0x00010000;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, format, 128, 128, 0, format, type, pData);
	...
	pData = (unsigned long *) (pIMG + (pIMG[0] / sizeof(unsigned long)));
